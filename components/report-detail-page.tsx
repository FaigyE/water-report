...
export function ReportDetailPage() {
  ...
  if (!reportData.installationData || !Array.isArray(reportData.installationData)) {
    return <div>Invalid installation data</div>
  }
  ...
  try {
    ...
    if (!groupedData || groupedData.length === 0) {
      return <div>No grouped data available</div>
    }
    ...
    return <div>Error processing report data: {String(error)}</div>
  }
}
